...
//                if (!measureGroupRange.contains(this.getRange())) continue;
			for (TabMeasure measure : tabRow.getMeasureList()) {
				...
				if (itWorked)
					measure.changesTimeSignature = true;
			}
	...
	public List<ValidationError> validate() {
		if (!(isTop)) {
			addError("Time signatures should only be applied to the top of measures.", 3, getRanges());
		} else if (beatCount <= 0 || beatType <= 0) {
			addError("Invalid beat " + (this.beatCount <= 0 ? "count" : "type") + " value.", 2, getRanges());
		} else if (!isValid(this.beatCount, this.beatType)) {
			addError("Unsupported time signature.", 2, getRanges());
		}
		return errors;
	}
	...
	public static boolean isValid(int beatCount, int beatType) {
		...
		if (!Arrays.stream(validDens).anyMatch(i -> i == beatType))
			result = false;
		return result;
	}
}
