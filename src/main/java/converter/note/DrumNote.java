...
public class DrumNote extends TabNote {
	...
	public DrumNote(int stringNumber, String origin, int position, String lineName, int distanceFromMeasure) {
		...
		// if ((drumPiece == DrumPiece.Bass_Drum_1) || (drumPiece ==
		// DrumPiece.Bass_Drum_2))
		// this.voice = 2;
	...
	public models.measure.note.Note getModel() {
		...
		if ((noteHead.equalsIgnoreCase("x"))
				|| ((noteHead.equalsIgnoreCase("o") && drumPiece == DrumPiece.OPEN_HI_HAT)))
			noteModel.setNotehead(new Notehead("x"));
		if (drumPiece == DrumPiece.RIDE_BELL)
			noteModel.setNotehead(new Notehead("diamond"));
		return noteModel;
	}
