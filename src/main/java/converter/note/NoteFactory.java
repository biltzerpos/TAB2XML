...
/**
 * A class to extract notes from single notes or combinations of notes e.g.
 * 6h8p6, or special types of notes like harmonics or grace notes
 */
public abstract class NoteFactory {
	 ...
	 *
	 */
	public List<TabNote> getNotes(int inputStringNumber, String inputNoteText, int inputPosition, String inputLineName,
			int inputDistance) {
		...
		if (noteRange.getStart() > 0) {
			noteList.add(new InvalidNote(stringNumber, noteText.substring(0, noteRange.getStart()), position, lineName,
					distanceFromMeasureStart));
		}

		if (noteRange.getEnd() < noteText.length()) {
			noteList.add(new InvalidNote(stringNumber, noteText.substring(noteRange.getEnd()), position, lineName,
					distanceFromMeasureStart + noteRange.getEnd()));
		}
	...
	protected List<TabNote> getNotesFromValidRange(Range r) {
		...
		TabNote note1;
		if (!noteMatcher.find()) {
			noteList.add(new InvalidNote(stringNumber, noteText.substring(idx, endIdx), position + idx, lineName,
					distanceFromMeasureStart + idx));
			return noteList;
		} else {
			List<TabNote> notes = createNote(noteMatcher.group(), position + idx + noteMatcher.start(),
					distanceFromMeasureStart + idx + noteMatcher.start());
			noteList.addAll(notes);
			note1 = notes.get(notes.size() - 1); // It is always the last note that builds a relationship. e.g you dont
													// wanna get the grace note. you wanna get the grace pair because it
													// is what will be creating a relationship with other notes
		}
	...
	protected boolean slur(TabNote note1, TabNote note2) {
		...
		note1.addDecorator((noteModel) -> {
			...
			if (note1.stringNumber < 4 && ((noteModel.getStem() == null) || (!noteModel.getStem().equals("up"))))
				slur.setPlacement("above");
			if (notations.getSlurs() == null)
				notations.setSlurs(new ArrayList<>());
