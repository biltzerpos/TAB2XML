...
public class TabRow extends ScoreComponent {
	...
	/**
	 * Creates a TabRow object from a List of AnchoredText which represent the lines
	 * in the tablature row
	 *
	 * @param data a List<AnchoredText> containing the lines which are meant to
	 *             represent a tablature row.
	 */
	public TabRow(List<AnchoredText> inputData) {
		...
		for (int i = 0; i < data.size(); i++) {
			...
			if (Settings.getInstance().getInstrument() == Instrument.GUITAR && i < 6) {
				if (currentNameData.text == "")
					currentNameData.text = Settings.getInstance().getGuitarTuning()[i][0]; // Keep using what ever
																							// tuning was previously set
																							// if this is guitar
				Settings.getInstance().setGuitarTuning(i, currentNameData.text.toUpperCase()); // Update tuning. Only
																								// likely to make a
																								// difference for the
																								// first measure
			}
			if (Settings.getInstance().getInstrument() == Instrument.BASS && i < 4) {
				if (currentNameData.text == "")
					currentNameData.text = Settings.getInstance().getBassTuning()[i][0]; // Keep using what ever tuning
																							// was previously set if
																							// this is bass
				Settings.getInstance().setBassTuning(i, currentNameData.text.toUpperCase()); // Update tuning. Only
																								// likely to make a
																								// difference for the
																								// first measure
			}
			...
			int measureCount = 0;
			Matcher measureInsidesMatcher = Pattern.compile(Patterns.insidesPattern())
					.matcher(currentLine.substring(nameOffset));
			while (measureInsidesMatcher.find()) {
				...
				// Get the particular measure we are interested in and add this line to its list
				// of lines
				AnchoredText currentLineData = new AnchoredText(measureText, measurePositionInScore,
						measurePositionInLine);
	...
	/**
	 * Creates an instance of the abstract Measure class whose concrete type depends
	 * on the instrument setting Also deals with in-measure repeat markings
	 */
	private TabMeasure from(List<AnchoredText> measureAT, List<AnchoredText> measureNameAT,
...
//        }
		...
		if (!countAT.text.isEmpty())
			repeatCount = Integer.parseInt(countAT.text);
		if (repeatStart)
	...
	private boolean checkRepeatStart(List<AnchoredText> atList) {
		...
		for (AnchoredText at : atList) {
			...
			if (at.text.strip().startsWith("|*"))
				repeatStartMarkCount++;
		}
	...
	private AnchoredText extractRepeatCount(List<AnchoredText> lines) {
		if (!checkRepeatEnd(lines))
			return new AnchoredText("", 0, 0);
		AnchoredText firstAT = lines.get(0);
		Matcher numMatcher = Pattern.compile("(?<=[^0-9])[0-9]+(?=[ ]|" + Patterns.DIVIDER + "|$)")
				.matcher(firstAT.text);
		if (!numMatcher.find())
			return new AnchoredText("", 0, 0);
		return new AnchoredText(numMatcher.group(), firstAT.positionInScore + numMatcher.start(), numMatcher.start());
	}

	private static void removeRepeatMarkings(List<AnchoredText> lines, boolean repeatStart, boolean repeatEnd,
			AnchoredText countAT) {
		if (!countAT.text.isEmpty()) {
			...
			line = line.substring(0, position) + "-".repeat(Math.max(numLen - 1, 0))
					+ line.substring(position + numLen);
			// remove extra - which overlaps with the |'s
			/*
			 * -----------4| -----------|| ----------*|| we wanna remove the -'s on the same
			 * column as the *'s. we do that for the first measure line in the code right
			 * below. the code lower handles the case for the rest of the lines.
			 * ----------*|| -----------|| -----------||
			 */
	...
	public AnchoredText nameOf(String line, int pos) {
		...
		if (measureLineNameMatcher.find())
			result = new AnchoredText(measureLineNameMatcher.group(), pos + measureLineNameMatcher.start(),
					measureLineNameMatcher.start());
		else
			result = null;
		return result;
	}
...
//    }
	...
	/**
	 * Validates if all Measure objects aggregated in this TabRow have the same
	 * number of measure lines. It also validates all its aggregates i.e all Measure
	 * objects and Instruction objects that it aggregates.
	 */
	@Override
	public List<ValidationError> validate() {
		...
		if (!hasEqualMeasureLineCount) {
			addError("All measures in a tablature row must have the same number of lines", 2, failPositions);
...
//	    }
		...
		// --------------Validate your aggregates (only if you're
		// valid)-------------------
		if (!errors.isEmpty())
			return errors;
