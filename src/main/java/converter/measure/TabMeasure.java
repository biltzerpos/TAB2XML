...
public abstract class TabMeasure extends ScoreComponent {
	...
	protected void createTabStringList() {
		...
		for (int i = 0; i < data.size(); i++) {
			...
			if (tabStringList.get(i).padding < padding)
				padding = tabStringList.get(i).padding;
		}
	}
	 ...
	 *
	 * @return a TabString either of type TabGuitarString, TabBassString, or
	 *         TabDrumString
	 */
	...
	/**
	 * Sets initial durations for the notes based on the number of characters they
	 * span. setDivisions() will modify these later
	 */
...
//									+ (divisions + adjustment) + " to " + divisions);
				...
				if (!Arrays.stream(supportedDivisions).anyMatch(i -> i == divisions / beatCount)) {
					// System.out.println("Measure " + measureCount + ": Unsupported divisions: " +
					// divisions);
			...
			for (List<List<TabNote>> voice : getVoiceSortedChordList()) {
				...
				for (List<TabNote> chord : voice) {
					if ((chord.get(0).mustSplit) && (chord.get(0).duration > 1)) {
						...
						int note1dur = firstNoteDuration(totalDuration, chord.get(0).duration, getDivisions(),
								getBeatType());
	...
	private int firstNoteDuration(int totalDuration, int duration, int divisions, int den) {
		...
		do {
			...
			// assert beatDuration > 0: "Measure " + measureCount +": beatDuration is " +
			// beatDuration;
	...
	/**
	 * @return the range of the first line of the measure in its text line If this
	 *         is the first measure in the line, the range includes the line name
	 */
	...
	/**
	 * Validates that all TabString objects have the same length. It also validates
	 * that timing was able to be determined successfully. It does not validate its
	 * aggregated objects. That job is left up to its concrete classes (this is an
	 * abstract class) TODO See if that validation can happen here
	 */
	@Override
	public List<ValidationError> validate() {

		if (unSupportedDivisions) {
			addError("Could not determine timing correctly: Unsupported divisions", 3, getRanges());
		}
		if (nonIntegerDivisions) {
			addError("Could not determine timing correctly: Non integer divisions", 3, getRanges());
		}
		if (split1) {
			addError("Could not determine timing correctly: Had to split a duration of 1", 3, getRanges());
		}
		if ((padding > allowedPadding) && (voiceSortedNoteList.size() > 0))
			addError("Empty space at the beginning of the measure will be ignored"
					+ "\nYou may want to add a rest (R) or a tied note (T) for correct timing", 2, getRanges());
		...
		if (!lineSizeEqual) {
			addError("Unequal measure line lengths may lead to incorrect note durations.", 2, getRanges());
		}
		return errors;
	}
