...
public class Patterns {
	...
	public static final String GUITAR_NOTE_GROUP_PATTERN = "(" + GUITAR_NOTE_PATTERN + "(-*" + GUITAR_NOTE_CONNECTOR
			+ "-*" + GUITAR_NOTE_PATTERN + ")*)";
	...
	public static final String DRUM_NOTE_CONNECTOR = "$a"; // there are no connectors, so this is a regex that never
															// matches anything. an a after the end of the string

	// e------------ or |e---------------- or |e|-------------------- when it is the
	// first measure of the measure group (start of line, SOL)
	...
	private static String nonDashDividerWhitespace() {
		// return "(?:" + NoteFactory.GUITAR_NOTE_PATTERN + "|" +
		// NoteFactory.GUITAR_NOTE_CONNECTOR + "|" + NoteFactory.DRUM_NOTE_PATTERN +
		// ")";
		return "[^-\\n" + DIVIDER_COMPONENTS + SPACEORTAB + "+]"; // Added + to exclude drum tab timing
	}

	/**
	 * A very general, very vague "inside a tab row line" pattern. We want to be as
	 * general and vague as possible so that we delay catching erroneous user input
	 * until we are able to pinpoint where the error is exactly. e.g. if this
	 * pattern directly detects a wrong note here, a Note object will never be
	 * created. It will just tell the user the measure line where the error is, not
	 * the precise note which caused the error. This regex pattern verifies if it is
	 * surrounded by |'s or a measure line name and captures a max of one | at each
	 * end only if it is surrounded by more than one | (i.e ||------| extracts
	 * |------ and ||------||| extracts |------|, but |------| extracts ------)
	 *
